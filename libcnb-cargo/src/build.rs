use crate::config::config_from_metadata;
...
) -> Result<BuildpackBinaries, BuildError> {
    let binary_target_names = binary_target_names(cargo_metadata);
    let config = config_from_metadata(cargo_metadata).unwrap();

    let cargo_env: HashMap<K, V> = cargo_env.into_iter().collect();
