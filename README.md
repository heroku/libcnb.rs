...
libcnb Cargo command which we will later use to package our buildpack:
...
eventually run on. This means we have to cross-compile our buildpack. The libcnb Cargo command tries to help you setting
...
As aforementioned, the buildpack we're writing will be very simple. We will just log a "Hello World" image during build
...
target platform, follow them and try again. These differ from platform to platform, that's why they're not part of this
quick start guide.
...
### Running the image
Just run the crated Docker image as you normally would. If all went well, you should see our "Hello World!" message in
your terminal:
